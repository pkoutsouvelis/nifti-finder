...
class TestComposeFilter:
    ...
    def test_and(self, file_paths):
        filter = ComposeFilter(
            [IncludeExtension("nii.gz"), IncludeFilePrefix("file")], logic="AND"
        )
        assert_filter(
            filter, file_paths, [False, True, True, False, False, False, False]
        )

    def test_and_w_exclude(self, file_paths):
        filter = ComposeFilter(
            [IncludeExtension("nii.gz"), ExcludeFilePrefix("file")], logic="AND"
        )
        assert_filter(filter, file_paths, [False, False, False, True, True, True, True])

    def test_or(self, file_paths):
        filter = ComposeFilter(
            [IncludeExtension("nii.gz"), IncludeFilePrefix("file")], logic="OR"
        )
        assert_filter(filter, file_paths, [True, True, True, True, True, True, True])

    def test_or_w_exclude(self, file_paths):
        filter = ComposeFilter(
            [IncludeExtension("nii.gz"), ExcludeFilePrefix("file")], logic="OR"
        )
    ...
    def test_nested_compose(self, file_paths):
        filter = ComposeFilter(
            [
                ...
                ComposeFilter(
                    [IncludeFilePrefix("prefix"), IncludeFileSuffix("suffix")],
                    logic="OR",
                ),
            ],
            logic="AND",
        ...
        assert_filter(
            filter, file_paths, [False, False, True, True, False, False, True]
        )
